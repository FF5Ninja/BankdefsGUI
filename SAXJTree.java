...
import cnuphys.bCNU.xml.tree.BadNodeException;
import cnuphys.bCNU.xml.tree.NodeProcessor;

/**
 * @author Heddle - A simple tree for displaying a xml document parsed via SAX
 * @author Brandon Rusk - Implemented into JTree GUI.
 */
@SuppressWarnings("serial")
public class SAXJTree extends JTree {

	public static JFrame frame;
	public static SAXJTreeStart example;
	 ...
	 * @throws SAXException
	 */
	public void buildTree(File file,String fname) throws IOException, SAXException, XMLStreamException {
		...
		try {
			...
			String filetype = fnameparts[fnameparts.length-1];
			...
			int response = JOptionPane.showConfirmDialog(null, error, "Error",
			...
			String error = "Please enter an .xml file.";
			int response = JOptionPane.showConfirmDialog(null, error, "Error",
			JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_OPTION);
		...
		for(int i=0;i<getRowCount();i++) {
			TreeNode node = (DefaultMutableTreeNode)getPathForRow(i).getLastPathComponent();
			String localName = node.toString();
			
			if(localName.matches("^[a-zA-Z]+\\ [a-zA-Z]+")) {
				collapseRow(i);
			}
		}
		for(int i=0;i<getRowCount();i++) {
			...
			if(!(localName.equals("column") || localName.equals("evio_dictionary"))) {
				collapseRow(i);
			}
		}
	}
	...
	class JTreeHandler extends DefaultHandler {
		...
		public void startElement(String namespaceURI, String localName,
			...
			if (localName.equals("section")) {
				for (int i = 0; i < atts.getLength(); i++)
				{
					if (atts.getLocalName(i).equals("info")) {
						localName = atts.getValue(i);
					}
				}
			}
			...
			String s = "";
			for (int i = 0; i < atts.getLength(); i++) {
				s += "(" + atts.getLocalName(i) + " = " + atts.getValue(i)
						+ ") ";
			}
